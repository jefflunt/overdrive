...
- **`crash/`**: Jobs that failed due to an error (non-zero exit code).
- **`no-op/`**: Jobs that completed but made no changes (detected by the agent).
- **`timeout/`**: Jobs that were terminated for exceeding the maximum execution time.
- **`stopped/`**: Jobs that were manually terminated by the user.
