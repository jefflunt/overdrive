...
- **OAuth Ready**: Integrated support for Google and GitHub authentication.
- **Multi-Project Management**: Isolated environments for different repositories with individual SSH configurations.
- **Job Search**: Quickly find previous jobs using the real-time fuzzy search interface.
- **Revert**: Easily revert specific jobs to a previous state.
- **Live Logs**: Watch engineering agents work in real-time with streaming ANSI-color logs.
- **Custom Environments**: Install project-specific Alpine Linux dependencies and configure persistent test databases (PostgreSQL, MySQL, SQLite).
- **Ephemeral Workers**: Secure execution in isolated containers that are cleaned up automatically.
- **Project Commands**: Define and execute custom scripts directly from the project interface with live output streaming.
- **Starring**: Highlight important Todo items with the new starring system.
